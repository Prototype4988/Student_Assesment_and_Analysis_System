...
module.exports = function (app, db, query_js) {
  ...
  function importExcelData2MySQLMarks(filePath, db, query_js) {
    ...
    readXlsxFile(filePath).then((rows) => {
      for (var i = 1; i < rows.length; i++) {
        let marks_details = {
          Course_Code: rows[i][0],
          Regulation: rows[i][1],
          Register_No: rows[i][2],
          student_name: rows[i][3],
          Department: rows[i][4],
          Semester: rows[i][5],
          Category: rows[i][6],
          Mark: rows[i][7],
          Staff_ID: rows[i][8],
          Course_ID: rows[i][0].concat(rows[i][4],rows[i][1],rows[i][8],rows[i][6],String(rows[i][2]).substring(4,6),String(rows[i][2]).substring(9)),
        };
